...
#warning "I haven't implemented this yet :("
  ...
  temp = _mm_mul_ps(
      temp, _mm_sub_ps(threehalfs, _mm_mul_ps(x2, _mm_mul_ps(temp, temp))));
...
real Vector::length() const { return sqrt(squareLength()); }

Vector Vector::random() {
  return Vector(vector_rand(random_generator), vector_rand(random_generator),
                vector_rand(random_generator));
}
...
#warning "I haven't implemented this yet :("
  ...
  // SPVO
  __m128 A = a.v, B = b.v;
  __m128 temp = _mm_sub_ps(_mm_mul_ps(A, _mm_shuffle_ps(B, B, shuf)),
                           _mm_mul_ps(_mm_shuffle_ps(A, A, shuf), B));
