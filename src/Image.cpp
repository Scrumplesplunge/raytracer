...
void Image::loadBMP(const char *filename) {
	...
	if (!file.good()) return error(0, "The file stream did not have the good-bit set after opening.");
	...
	if (!file.good()) return error(1, "The file stream was not good after reading the file header @(0x0, 14)");

	// Check that the file we are reading is, in fact, a bitmap.
	if (!(tmp[0] == 'B' && tmp[1] == 'M')) return error(1, "The first two bytes did not match 'BM'.");

	// Read the address that the image data starts at.
	unsigned int offset = ((unsigned int) tmp[10]) | (((unsigned int) tmp[11]) << 8) | (((unsigned int) tmp[12]) << 16) | (((unsigned int) tmp[13]) << 24);

	// Read the bitmap header size value.
	file.read((char*) tmp, 4);
	if (!file.good()) return error(1, "The file stream was not good after reading the DIB header size @(0xE, 4)");
	unsigned int bitmapHeaderSize = ((unsigned int) tmp[0]) | (((unsigned int) tmp[1]) << 8) | (((unsigned int) tmp[2]) << 16) | (((unsigned int) tmp[3]) << 24);
	
	// Throw an error if the header size is something preposterous.
	if (bitmapHeaderSize > 128) return error(3, "The specified bitmap header size exceeds 128, which (at the time of writing) seems silly.");

	// I'm a combination of lazy and naive so I'm only supporting the 40-byte BITMAPINFOHEADER.
	// What you gonna do - sue me?
	if (bitmapHeaderSize != 40) return error(4, "This humble image loader only supports the 40-byte BITMAPINFOHEADER for the DIB header.\nBe a lamb and convert your image to use this format please.");

	// Load the image dimensions.
	file.read((char*) tmp, 8);
	if (!file.good()) return error(1, "The file stream was not good after reading the image dimensions @(0x12, 8)");

	// The values are signed, to allow the rows to be stored bottom-to-top or top-to-bottom etc.
	int signedWidth = (int)(((unsigned int) tmp[0]) | (((unsigned int) tmp[1]) << 8) | (((unsigned int) tmp[2]) << 16) | (((unsigned int) tmp[3]) << 24));
	int signedHeight = (int)(((unsigned int) tmp[4]) | (((unsigned int) tmp[5]) << 8) | (((unsigned int) tmp[6]) << 16) | (((unsigned int) tmp[7]) << 24));
	
	// Perform sanity checks on these values.
	if (signedWidth < 0) return error(3, "Whilst negative heights have got a rational justification, negative widths are just silly.");
	if (signedWidth == 0) return error(3, "There is literally no point in having a bitmap with a width of 0.");
	if (signedHeight == 0) return error(3, "There is literally no point in having a bitmap with a height of 0.");
	...
	file.read((char*) tmp, 8);
	if (!file.good()) return error(1, "The file stream was not good after reading the configuration details @(0x1A, 8)");

	if ((((unsigned int) tmp[0]) | (((unsigned int) tmp[1]) << 8)) != 1)
		return error(3, "The number of colour planes is literally defined in the specification as 1.");

	unsigned int bitsPerPixel = ((unsigned int) tmp[2]) | (((unsigned int) tmp[3]) << 8);
	if (bitsPerPixel != 24) return error(4, "This humble image loader only supports 24-bit colour values per pixel.");
	unsigned int compression = ((unsigned int) tmp[4]) | (((unsigned int) tmp[5]) << 8) | (((unsigned int) tmp[6]) << 16) | (((unsigned int) tmp[7]) << 24);
	if (compression != 0) return error(4, "This humble image loader does not support pixel compression.");

	// Load the image buffer size.
	file.read((char*) tmp, 4);
	if (!file.good()) return error(1, "The file stream was not good after reading the image buffer size @(0x22, 4)");

	unsigned int dataSize = ((unsigned int) tmp[0]) | (((unsigned int) tmp[1]) << 8) | (((unsigned int) tmp[2]) << 16) | (((unsigned int) tmp[3]) << 24);

	// Quite a simple sanity check for the data size.
	if (dataSize < 3 * width * height) return error(3, "The image buffer size is smaller than the amount of required data.");

	// We don't actually care about the rest of the bitmap header for the purposes of this image loader.
	file.seekg(offset);

	// Just load the image data, remove the padding, and put it in the image buffer.
	...
	if (!file.good()) {
		...
		return error(1, "The file stream was not good after reading the image data @(<offset>, <dataSize>)");
	}
...
bool Image::saveBMP(const char *filename) {
	...
	char pad[3] = {'L', 'O', 'L'};	// These are the padding bytes that fill in the end of each row.
	for (unsigned int i = 0; i < height; i++) {
...
Image::Image(unsigned int w, unsigned int h) : success(true), errorString(NULL), detailedErrorString(NULL), width(w), height(h), data(new unsigned char[width * height * 3]) {}

Image::Image(const char *filename) : success(false), errorString(NULL), detailedErrorString(NULL), width(0), height(0), data(NULL) {
	load(filename);
}

Image::Image(const Image& prototype) : success(false), errorString(NULL), detailedErrorString(NULL), width(0), height(0), data(NULL) {
	clone(prototype);
}
...
void Image::operator=(const Image& prototype) {
	clone(prototype);
}
...
void Image::load(const char *filename) {
	...
	// Format unsupported.
	return error(5, "Cannot load the specified file as its extension is unrecognised/unsupported.");
}
...
bool Image::save(const char *filename) {
	...
	// Format unsupported.
	error(5, "Cannot save to the specified file as its extension is unrecognised/unsupported.");
	return false;
}
